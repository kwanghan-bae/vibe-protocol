...
도구는 두 가지로 분류됩니다: **에이전트가 사용하는 도구**와 **당신의 생산성을 높이는 도구**.

### 🤖 에이전트용 도구 (Agent Tools)

**AI 에이전트가 코드를 분석하고 자동화 작업에 사용하는 CLI 도구들**
...
**왜 중요한가?**
- AI 에이전트는 이 도구들을 사용하여 당신의 코드를 0.1초 만에 파악합니다
...
**개발자(당신)의 생산성을 극대화하는 TUI/인터랙티브 도구들**

| 도구 | 대체 대상 | 역할 | 왜 설치해야 하는가? |
|:---|:---|:---|:---|
| **`lazygit`** | `git` CLI | **Git GUI (TUI)** | 복잡한 Git 명령어를 직관적인 UI로 해결. 충돌 해결과 리베이스가 미친듯이 빨라짐. |
| **`zoxide`** | `cd` | **스마트 이동** | 자주 가는 폴더를 기억. `z work` 입력만으로 `/Users/me/git/work`로 점프! |
| **`fzf`** | `Ctrl+R` | **퍼지 검색** | 히스토리 검색, 파일 찾기가 훨씬 빨라짐. 오타 허용. |
| **`atuin`** | `Ctrl+R` | **히스토리 검색** | 모든 명령어를 SQLite에 저장. 디렉토리별, 시간대별 필터링 가능. |
| **`btm`** | `top` | **모니터링 (TUI)** | `htop`보다 가볍고 예쁨. 내 맥북이 왜 뜨거운지 1초 만에 파악. |
| **`dust`** | `du` | **용량 분석** | 디스크 용량이 어디서 세는지 트리맵으로 시각화. |
| **`duf`** | `df` | **디스크 확인** | 파티션 정보를 엑셀 표처럼 깔끔하게 표시. |

> [!TIP]
> **Zsh 플러그인 추천**:
> - `zsh-autosuggestions` - 타이핑하면서 이전 명령어 자동 제안
> - `zsh-syntax-highlighting` - 명령어 오류를 색상으로 즉시 확인
> - `starship` - 빠르고 예쁜 프롬프트 (Git 상태 자동 표시)
>
> 상세 설치 가이드: [TERMINAL_SETUP.md](docs/TERMINAL_SETUP.md)

**왜 중요한가?**
- 터미널에서 작업하는 시간이 50% 줄어듭니다
- Git, 파일 탐색, 시스템 모니터링이 마우스 없이 가능해집니다
- 한번 익숙해지면 GUI보다 훨씬 빠릅니다
